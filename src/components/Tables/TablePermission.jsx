...
const TablePermission = (props) => {
  ...
  const [PermissionsRoleValue, setPermissionsRoleValue] = useState([]);
  const [OpenDialogDelete, setOpenDialogDelete] = React.useState(false);

  //  Get permission and stock permission in state and open dialog delete
  const getPermissionId = (id) => {
    setOpenDialogDelete(true);
    props.infoCondidate.forEach((element) => {
      if (element.id == id) {
        setPermissionsRoleValue(element);
      }
    });
  };
  // close dialog delete
  const handleCloseDialogDelete = (e) => {
    ...
    console.log('deleted pemission ==> ',e)
  };
  ...
  // close dialog update
  const handleCloseDialogUpdate = (id , name) => {
    setOpenDialogUpdate(false);
    console.log('updated permission ===> ',id, '===' , name) 
  };
            ...
            {props.infoCondidate.map(({ id, name }, index) => {
              return (
                <tr key={id} className="border_bottom_gray">
                  ...
                  <td className="p-4 cursor-pointer ">
                    ...
                    >
                      ...
                      <DialogFooter className="space-x-2">
                        ...
                        <button
                          className="middle none center rounded-lg py-2 px-4 font-sans text-xs font-bold uppercase text-gray-700 transition-all hover:bg-gray-500/10 active:bg-gray-500/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                          onClick={()=>handleCloseDialogUpdate(PermissionsRole.id ,PermissionsRole.name )}
                        >
                          update
                        </button>
                      </DialogFooter>
                  ...
                  {/* delete permission */}
                  <td className="p-4 cursor-pointer ">
                    <div onClick={() => getPermissionId(id)}>
                    ...
                    >
                      ...
                      <DialogFooter className="space-x-2">
                        ...
                        <button
                          className="middle none center rounded-lg py-2 px-4 font-sans text-xs font-bold uppercase text-gray-700 transition-all hover:bg-gray-500/10 active:bg-gray-500/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                          onClick={()=>handleCloseDialogDelete(PermissionsRoleValue.id)}
                        >
                          delete
                        </button>
                      </DialogFooter>
